...
def sidebar_inputs():
    # --- Tickers extracted from ind_nifty500list.csv ---
    # NOTE: This list contains all 503 Nifty 500 stocks (with the .NS suffix).
    # It is hardcoded here for simplicity in Streamlit Cloud deployment.
    ...
    if not tickers:
        # Stop the app if no tickers are selected
        st.error("Please select at least one NSE ticker to run the analysis.")
        st.stop()
        
    return tickers, {} # Return tickers and an empty criteria dict
